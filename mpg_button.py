...
class GearSelector(QWidget, HALWidget, VCPWidget):
    """Seletor de 16 marchas (mutuamente exclusivo) com saida em 4 pinos sel0..sel3.

    Cada marcha mapeia para um indice 0..15 codificado em binario (LSB = sel0)
    para alimentar diretamente os pinos sel0..sel3 de mux16.

    HAL pins:
      .sel0 / .sel1 / .sel2 / .sel3  (bit out) - indice da marcha em binario
      .spinning                      (bit in)  - desabilita troca enquanto spindle gira
    Estado salvo entre sessoes em QSettings(DinoEvo/Gearbox16).
    """
    ...
    def __init__(self, parent=None):
        ...
        self._settings = QSettings("DinoEvo", "Gearbox16")
    ...
    def _publish(self, idx):
        ...
        if self._sel3_pin is not None:
            self._sel3_pin.value = bool(idx & 0x8)
    ...
    def initialize(self):
        ...
        self._sel3_pin = comp.addPin(obj_name + ".sel3", "bit", "out")
        self._spinning_pin = comp.addPin(obj_name + ".spinning", "bit", "in")
        self._spinning_pin.valueChanged.connect(self._on_spinning_changed)
